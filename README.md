...
ヘッダーの共有ボタンから、3つの本棚を閲覧専用で開ける共有リンクを作成してコピーできます。共有データはURL内へ圧縮して保存され、サーバーやデータベースには保存されません。タイトル・著者・表紙・配置・本棚名だけが含まれ、読了日・評価・感想・ISBN・出版社は含まれません。リンクを知っている人は内容を閲覧できるため、貼り付け先の公開範囲に注意してください。

共有リンクには、実際のアプリの本棚画面を使った1200×630のプレビュー画像を設定しています。対応するサービスへURLを貼り付けると、リンクの上に画像付きカードが表示されます。GitHub PagesはURLのフラグメント（`#share=...`）ごとにHTMLを生成できないため、プレビュー画像は共有者ごとの本棚ではなく、アプリ共通の本棚画面です。
